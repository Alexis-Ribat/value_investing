...
if df.empty:
    ...
    with tab1:
        ...
        with c2:
            ...
            fig_gauge = go.Figure(go.Indicator(
                ...
                delta={'reference': price, 'relative': True},
                gauge={
    ...
    with tab4:
        ...
        # Choix de la source
        analysis_mode = st.radio("Source des données :", ["📰 Dernières News (Auto)", "📝 Transcription Complète (Manuel)"], horizontal=True)
        ...
        if analysis_mode == "📰 Dernières News (Auto)":
            ...
            if st.button("🔍 Scanner les News & Analyser"):
                with st.spinner(f"Récupération des articles pour {current_ticker}..."):
                    try:
                        # --- REMPLACEMENT DE YAHOO PAR GOOGLE NEWS RSS (Plus robuste) ---
                        # On utilise le flux RSS officiel de Google News
                        url = f"https://news.google.com/rss/search?q={current_ticker}+stock+finance&hl=en-US&gl=US&ceid=US:en"
                        
                        response = requests.get(url, timeout=5)
                        soup = BeautifulSoup(response.content, features="xml") # "xml" ou "html.parser"
                        ...
                        for item in items[:10]:
                            title = item.title.text
                            pub_date = item.pubDate.text
                            # Le lien peut servir si on veut aller plus loin, mais le titre contient souvent l'info clé
                            full_text += f"- Titre : {title}\n- Date : {pub_date}\n\n"
                            count += 1
                        
                        if count > 0:
                            text_to_analyze = full_text
                            st.success(f"{count} articles récupérés via Google News.")
                            
                            with st.expander("Voir les titres bruts récupérés"):
                                st.text(full_text)
                            
                            # Appel IA
                            with st.chat_message("assistant"):
                                from src.ai_engine import analyze_earnings_sentiment
                                response = analyze_earnings_sentiment(current_ticker, text_to_analyze)
                                st.markdown(response)
                        else:
                            st.error("Aucune news trouvée. Essayez le mode Manuel.")
                            
                    except Exception as e:
                        st.error(f"Erreur lors de la récupération des news : {e}")

        else: # Mode Manuel
            st.caption("Copiez-collez ici la transcription de l'Earnings Call (trouvée sur SeekingAlpha ou le site Investisseur).")
            raw_transcript = st.text_area("Collez le texte ici", height=300, placeholder="Operator: Good day and welcome to the Apple Q3 Earnings Conference Call...")
            
            if st.button("🧠 Analyser la Transcription"):
                if len(raw_transcript) > 100:
                    with st.spinner("Lecture et analyse psychologique en cours..."):
                        with st.chat_message("assistant"):
                             # Remplacez par votre fonction d'appel importée
                            from src.ai_engine import analyze_earnings_sentiment
                            response = analyze_earnings_sentiment(current_ticker, raw_transcript)
                            st.markdown(response)
                else:
